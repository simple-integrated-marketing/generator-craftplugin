...
module.exports = yo.generators.Base.extend({
  ...
  initializing: function() {
    ...
    if (fs.existsSync(PLUGIN_CONF_FILE_NAME)) {
      ...
      for (var property in obj) {
        if (obj.hasOwnProperty(property)) {
          ...
          this.options[property] = [this.options[property], attributeValue]
            .filter(function(val) {
              return val;
            })
            .join(",");
    ...
    if (this.askApiVersion) {
      ...
      this.api = apis["api_version_3_0"];
      this.apiVersion = "api_version_3_0";
      /* -- Change the templates root based on the API version */
      this.sourceRoot(this.sourceRoot() + "/" + this.api.API_KEY);
      this.log(chalk.yellow("🤖 Default to Craftcms 3.x mode..."));
      done();
    }
  },
  ...
  prompting: function() {
    ...
    this._optionOrPrompt(
      ...
      function(answers) {
        ...
        if (
          this.api.API_KEY == "api_version_3_0" ||
          this.api.API_KEY == "module_api_version_3_0"
        ) {
          // Make sure this isn't a reserved word
          if (
            phpReservedWords.indexOf(this.answers.pluginName.toLowerCase()) !=
            -1
          ) {
            console.log(
              "### Invalid use of a PHP reserved word as a plugin name: " +
        ...
        if (
          this.api.API_KEY == "module_api_version_3_0" &&
          !this.answers.pluginDirName.includes("module")
        ) {
        ...
        this.answers.codeComments = false;

        this.answers.dateNow = now.toISOString();
        this.answers.niceDate = now.yyyymmdd();

        this.answers.copyrightNotice =
          "Copyright (c) " +
          now.getFullYear() +
          " " +
          this.answers.pluginAuthorName;
        ...
        if (this.answers.pluginAuthorGithub) {
          this.answers.pluginDownloadUrl =
            "https://github.com/" +
            this.answers.pluginAuthorGithub +
            "/" +
            this.answers.pluginDirName +
            "/archive/master.zip";
          this.answers.pluginCloneUrl =
            "https://github.com/" +
            this.answers.pluginAuthorGithub +
          ...
          if (
            ...
            this.answers.pluginChangelogUrl =
              ...
              this.answers.pluginAuthorGithub +
              "/" +
              this.answers.pluginKebabHandle +
              "/master/CHANGELOG.md";
        ...
        if (this.answers.codeComments.constructor === Array) {
          this.answers.codeComments =
            typeof this.answers.codeComments !== "undefined" &&
            this.answers.codeComments.length > 0;
        }
        

        /* -- Clean up the various handle names, and convert them to arrays */

        var subPrefixHandles = [
          "controllerName",
          "elementName",
          "fieldName",
          "modelName",
        ...
        subPrefixHandles.forEach(function(subElement) {
          if (typeof _this.answers[subElement] !== "undefined") {
            ...
            if (_this.api.API_KEY == "api_version_2_5") {
              _this.answers[subElement].forEach(function(
                nameElement,
                nameIndex,
                nameArray
              ) {
            ...
            if (_this.api.API_KEY == "api_version_3_0") {
              _this.answers[subElement].forEach(function(
                ...
                nameArray[
                  nameIndex
                ] = nameElement.camelize().capitalizeFirstLetter();
              });
            }
        ...
        if (
          _this.api.API_KEY == "api_version_3_0" ||
          _this.api.API_KEY == "module_api_version_3_0"
        ) {
          if (typeof _this.answers["cpsectionName"] !== "undefined") {
            // Special-case for cpsections so that the default is "index"
            if (
              _this.answers["cpsectionName"].length == 1 &&
              _this.answers["cpsectionName"][0] == ""
            ) {
          ...
          defaultNameHandles.forEach(function(defaultNameElement) {
            if (typeof _this.answers[defaultNameElement] !== "undefined") {
              _this.answers[defaultNameElement].forEach(function(
                nameElement,
                nameIndex,
                nameArray
              ) {
                var defName =
                  _this.answers.pluginHandle +
                  defaultNameElement.capitalizeFirstLetter().slice(0, -4);
                // Special-case for cpsections so that the default is "Default"
                if (
                  defaultNameElement == "controllerName" ||
                  defaultNameElement == "consolecommandName"
                ) {
                  defName = "Default";
                }
                if (nameElement == "") {
                ...
                if (phpReservedWords.indexOf(nameElement.toLowerCase()) != -1) {
                  console.log(
                    "### Invalid use of a PHP reserved word as a component name: " +
                      nameElement
                  );
                  nameArray[nameIndex] =
                    nameElement +
  ...
  writing: function() {
    ...
    for (var i = 0; i < this.api.TEMPLATE_FILES.length; i++) {
      ...
      if (
        file.subPrefix &&
        typeof this.answers[file.subPrefix] == "undefined"
      ) {
        skip = true;
      }
      ...
      if (!skip) {
        if (file.prefix) {
          ...
          if (file.subPrefix) {
            ...
            if (file.dirSubPrefix) {
              dirPrefix =
                this.answers.pluginHandle.directorize() + file.dirSubPrefix;
            }
            if (file.kebabcasePrefix)
              ...
              destFile =
                ...
                file.destDir +
                dirPrefix +
                this.answers.pluginHandle +
                file.dest;
            // Only write it if the file doesn't exist already
            if (!fs.existsSync(destFile)) {
              this.log(
                "+ " +
                  this.answers.templatesDir +
                  "/" +
                  file.src +
                  " wrote to " +
                  chalk.green(destFile)
  ...
  /* -- end - Called last, cleanup, say good bye, etc */
    ...
    this.log(
      "Your Craft CMS plugin " +
        chalk.green(this.answers.pluginHandle) +
        " has been created."
    );
    this.log(
      "The default LICENSE.txt is the " +
        chalk.green("MIT license") +
...
function newClosure(whatsRequired) {
  ...
  return function(answers) {
    return typeof answers.pluginComponents != "object"
      ? false
      : answers.pluginComponents.indexOf(whichWhen) != -1;
  };
}
...
String.prototype.prefixize = function() {
  if (this == "") return this;
  else return "_" + this.camelize().capitalizeFirstLetter();
};
